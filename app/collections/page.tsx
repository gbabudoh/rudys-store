...
export default function CollectionsPage() {
  ...
  // Get real products - filter for clothing only
  ...
  const genders = Array.from(new Set(collectionsProducts.map(p => p.gender))).filter(Boolean);
  const brands = Array.from(new Set([...collectionsProducts.map(p => p.brand), 'Adidas', 'Nike'])).filter(Boolean).sort();
  const allSizes = Array.from(new Set(collectionsProducts.flatMap(p => p.sizes))).filter(Boolean).sort();
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <div className="inline-block mb-4 px-4 py-2 rounded-full backdrop-blur-sm border" style={{ backgroundColor: 'rgba(207, 162, 36, 0.2)', borderColor: '#cfa224' }}>
              <span className="text-sm font-semibold" style={{ color: '#cfa224' }}>Premium Fashion</span>
            </div>
            ...
            <p className="text-xl lg:text-2xl max-w-3xl mx-auto leading-relaxed" style={{ color: 'rgba(255, 255, 255, 0.9)' }}>
              Discover our premium fashion collection featuring the latest trends and timeless classics
            </p>
          </div>
        </div>
      </div>
