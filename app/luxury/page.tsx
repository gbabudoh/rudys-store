...
const categories = ['All', 'Bags', 'Glasses', 'Dresses'];
...
export default function LuxuryPage() {
  ...
  const genders = Array.from(new Set(luxuryProducts.map(p => p.gender))).filter(Boolean);
  const brands = Array.from(new Set(luxuryProducts.map(p => p.brand))).filter(Boolean);
  const allSizes = Array.from(new Set(luxuryProducts.flatMap(p => p.sizes))).filter(Boolean).sort();
  const subcategories = Array.from(new Set(luxuryProducts.map(p => p.subcategory))).filter(Boolean);
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-xl lg:text-2xl max-w-3xl mx-auto leading-relaxed" style={{ color: 'rgba(255, 255, 255, 0.9)' }}>
              Indulge in our exclusive collection of luxury items and premium accessories
            </p>
          </div>
        </div>
      </div>
