...
export default function ShippingPage() {
  ...
  useEffect(() => {
    fetch('/api/content-pages/shipping-info')
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center text-white">
            ...
            <p className="text-xl md:text-2xl" style={{ color: 'rgba(255, 255, 255, 0.9)' }}>
              {content?.content?.split('\n\n')[0]?.replace(/###\s/, '') || "Fast, reliable delivery to your doorstep"}
            </p>
          </div>
        </div>
      </section>
            ...
            {content?.content && content.content.split('\n\n').length > 1 ? (
              ...
              <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12 border-l-8" style={{ borderColor: '#cfa224' }}>
                ...
                <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12 text-center">
                  <p className="text-gray-600">No shipping information content available yet.</p>
                </div>
            )}
          </div>
        </div>
