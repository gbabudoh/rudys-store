...
export default function ClothingSizesManagement() {
  ...
  const fetchSizes = async () => {
    try {
      ...
      const response = await fetch('/api/admin/clothing-sizes', {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch(`/api/admin/clothing-sizes/${sizeToDelete}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
  ...
  const handleSaveSize = async (data: Partial<ClothingSize>) => {
    try {
      const token = localStorage.getItem('admin_token');
      const method = editingSize ? 'PUT' : 'POST';
      const url = editingSize ? `/api/admin/clothing-sizes/${editingSize.id}` : '/api/admin/clothing-sizes';
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center bg-white rounded-lg p-4 border border-gray-100 shadow-sm">
        <div>
          <h1 className="text-lg font-semibold text-gray-900">Clothing Sizes</h1>
          <p className="text-gray-500 text-[13px] mt-0.5">Manage available sizes for clothing items</p>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50/50 border-b border-gray-100">
              <tr>
                <th className="px-4 py-2.5 text-[13px] font-semibold text-gray-400 uppercase tracking-wider">Size Name</th>
                <th className="px-4 py-2.5 text-[13px] font-semibold text-gray-400 uppercase tracking-wider">Display Order</th>
                ...
                <tr key={size.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-4 py-2.5">
                    <div className="flex items-center">
                      <div className="w-7 h-7 rounded-md bg-blue-50 flex items-center justify-center mr-3 group-hover:bg-blue-100 transition-colors border border-blue-100">
                        <Ruler className="w-3.5 h-3.5 text-blue-600" />
                      </div>
                      <span className="text-[13px] font-semibold text-gray-900">{size.name}</span>
                    </div>
                  </td>
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-12 text-center text-gray-500">
                     <div className="flex flex-col items-center">
                      ...
                      <p className="text-[13px] font-semibold text-gray-900">No clothing sizes found</p>
...
function SizeModal({ onClose, onSave, size }: {
  onClose: () => void;
  onSave: (data: Partial<ClothingSize>) => void;
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={size ? 'Edit Clothing Size' : 'Add New Clothing Size'}
      width="max-w-lg"
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block text-[13px] font-black text-gray-900 mb-2 uppercase tracking-wider">Size Name</label>
          <input
            type="text"
            required
