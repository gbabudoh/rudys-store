...
export async function GET(request: NextRequest) {
  try {
    ...
    const bg = searchParams.get('bg') || '#f8fafc';
    ...
    const svg = `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="50%" y="60%" text-anchor="middle" dy=".3em" font-family="system-ui, -apple-system, sans-serif" font-size="${Math.min(width, height) / 20}" fill="${color}" font-weight="500">
        ${text}
      </text>
    </svg>`;
