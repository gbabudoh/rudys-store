...
export default function CrocsPage() {
  ...
  const [crocsProducts, setCrocsProducts] = useState<Product[]>([]);
  const [loadingProducts, setLoadingProducts] = useState(true);

  const fetchProducts = useCallback(async () => {
    try {
      setLoadingProducts(true);
      const response = await fetch('/api/products?store_section=crocs');
      if (response.ok) {
      ...
      console.error('Error fetching crocs products:', error);
    } finally {
  ...
  const genders = Array.from(
    new Set(crocsProducts.map((p) => p.gender)),
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            >
              Explore our wide range of comfortable and stylish Crocs
            </p>
          </div>
        </div>
      </div>
